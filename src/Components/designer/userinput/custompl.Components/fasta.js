...
export default function Fasta(props) {
  async function ValidateFileUpload (dataString, filename){
    ...
    const response = await fetch('http://127.0.0.1:5000/fileupload/fasta?file='+JSON.stringify(dataString));
    const result = await response.json()
    if(result.seq){
      props.setUploadedFile({seq:result.seq,label:filename,collection:'',type:'fasta',base64:dataString})
      props.setCatchError('')
    } else {
